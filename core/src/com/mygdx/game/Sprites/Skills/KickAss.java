...
/**
 * Created by arnat on 15/7/2560.
 */

public class KickAss extends Sprite { //Sprite คือรูปภาพที่เราจะนำมาใช้ในเกมเป็นภาพของวัตถุ (Object) นำมากำหนดการกระทำหรือแสดงไว้เฉยๆเพื่อประกอบในเกม
    ...
    public KickAss(PlayScreen screen, float x, float y, boolean fireRight){//คอนสตัคเตอร์ FireBall รูป,ตำแหน่งx,ตำแหน่ง y, วิ่งไปซ้ายหรืขวา
        ...
        defineKickAss();//กำหนดลูกไฟ
    }

    public void defineKickAss(){//กำหนดลูกไฟ
        ...
        bdef.type = BodyDef.BodyType.StaticBody;//ขนิดเคลื่อนที่ได้//KinematicBody แบบลอย
        ...
        if(fireRight) {
            ...
            PolygonShape shape = new PolygonShape(); //รูปหลายเหลี่ยม
            Vector2[] vertice = new Vector2[4]; //รัศมี 4 จุด ตัดมุมสี่เหลี่ยม
            vertice[0] = new Vector2(-4, 2).scl(1 / GameFarmer.PPM);
            vertice[1] = new Vector2(-4, -5).scl(1 / GameFarmer.PPM);
            vertice[2] = new Vector2(10, -2).scl(1 / GameFarmer.PPM);
            vertice[3] = new Vector2(10, 5).scl(1 / GameFarmer.PPM);
            shape.set(vertice);//เซ็ตรัศมีหัว
            fdef.shape = shape;
        }else {
            PolygonShape shape = new PolygonShape(); //รูปหลายเหลี่ยม
            Vector2[] vertice = new Vector2[4]; //รัศมี 4 จุด ตัดมุมสี่เหลี่ยม
        ...
        fdef.filter.categoryBits = GameFarmer.KICKASS_BIT;//categoryBits บิตหมวดหมู่การชนกัน กำหนดส่วน = GameFarmer.MARIO_BIT
        ...
        fdef.restitution =0.5f;//ค่าการเด้ง
      ...
      //  b2body.setLinearVelocity(new Vector2(fireRight ? 0 : -0, 1.5f)); //กำหนดการเคลื่อนไหวของไฟ//setLinearVelocity กำหนดความเร็วเชิงเส้น
    ...
    public void update(float dt){
        ...
        if((stateTime > 0.1f || setToDestroy) && !destroyed) {// (ถ้าเวลาเเกิน >3 หรือ || กำหนดไปทำลาย) && ไม่ได้ทำลาย
